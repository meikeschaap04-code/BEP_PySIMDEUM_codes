...
SCENARIOS = {
    ...
    'Scenario 5a': {
        'description': 'Centrale cap 15,6 L/min + decentraal (kranen 5, douche 7,8 L/min)',
        'modifications': {
            'type':            'centrale_decentraal',
            'cap_lps':         15.6 / 60.0,
            'tap_cap_lps':     5.0 / 60.0,
            'shower_cap_lps':  7.8 / 60.0,
        },
    ...
    'Scenario 5b': {
        'description': 'Centrale cap 17,8 L/min + decentraal (kranen 5, douche 7,8 L/min)',
        'modifications': {
            'type':            'centrale_decentraal',
            'cap_lps':         17.8 / 60.0,
            'tap_cap_lps':     5.0 / 60.0,
...
def apply_centrale_decentraal(consumption: xr.DataArray,
                              cap_lps: float,
                              tap_cap_lps: float,
                              shower_cap_lps: float) -> xr.DataArray:
    after_centrale = apply_centrale_begrenzing(consumption, cap_lps)
    return apply_decentrale_begrenzing(after_centrale, tap_cap_lps, shower_cap_lps)
...
def apply_modifications(consumption: xr.DataArray, modifications: Dict) -> xr.DataArray:
    ...
    elif mod_type == 'centrale_decentraal':
        return apply_centrale_decentraal(
            consumption,
            cap_lps=modifications['cap_lps'],
            tap_cap_lps=modifications['tap_cap_lps'],
            shower_cap_lps=modifications['shower_cap_lps'],
        )
    return consumption
    ...
    for day_idx in range(num_days):
        ...
        try:
            ...
            if (cfg['modifications'] is not None
                    and cfg['modifications']['type'] in ('centrale_begrenzing', 'centrale_decentraal')):
...
if __name__ == "__main__":
    ...
    _style_ax(ax)
    plt.tight_layout()
    plot_path_1 = r'C:\Users\meike\OneDrive\Documenten\Claude\Projects\Bep Meike\convergentie_100dagen14juni.png'
    plt.savefig(plot_path_1, dpi=150, bbox_inches='tight')
    print(f"Plot 1 opgeslagen: {plot_path_1}")
    plt.show()
    ...
    ax.set_title(f'Convergentie waterverbruik per scenario ({HOUSE_TYPE}) — alle {NUM_DAYS} dagen', fontsize=13)
    _style_ax(ax)
    plt.tight_layout()
    plot_path_2 = r'C:\Users\meike\OneDrive\Documenten\Claude\Projects\Bep Meike\convergentie_alledagen14juni.png'
    ...
    print(f"\nTotale runtime: {int(hours)}u {int(minutes)}m {seconds:.2f}s")
